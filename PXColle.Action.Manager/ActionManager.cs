...
namespace PXColle.Action.Manager
{
    public class ActionManager
    {
        public event EventHandler<OnStatusChangeEventArgs> OnStatusChange;
        private List<IPXAction> actions = new List<IPXAction>();
        static LimitedConcurrencyLevelTaskScheduler scheduler = new LimitedConcurrencyLevelTaskScheduler(3);
        ...
        {
            var action = actions.Find(x => x.context.Id == id);
            if (action != null)
            {
                Start(action);
            }
        ...
        public void Restart(string id)
        {
            ...
            if (action != null)
            {
                action.context.Status = PXActionStatus.Pending;
                //TODO
        ...
        {
            actions.Find(x => x.context.Id == id)?.Stop();
        }
        ...
        public IEnumerable<PXActionContext> List()
        {
            return actions.Select(x => x.context);
        }
